...
class Sector extends CI_Controller {
    ...
    function search_data() {
        ...
        $this->load->view('master/sector/sek_view_page', $result);
    }

    function select_sector(){
        $id = $this->input->post('sector_id');
        //$id = '2';
        $result['option_sector'] = $this->sek_model->getList($id);
        $this->load->view('master/sector/sector', $result);
    }
    ...
    function addData(){
		...
		$result['query']['id'] = '';
        $result['query']['name'] = '';
        $this->load->view('master/sector/sek_form_page',$result);
    }
